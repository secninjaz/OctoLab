...
class ReviewViewHolder
    ...
    public void bind(TimelineItem.TimelineReview item) {
        GitLabReview review = item.review();
        ...
        formatTitle(review);
        mImageGetter.bindMarkdown(mBodyView, review.body(), review.id());
        ...
        if (mDisplayReviewDetails && hasDiffs) {
            ...
            for (TimelineItem.Diff diffHunk : item.getDiffHunks()) {
                GitLabComment commitComment = diffHunk.getInitialComment();
    ...
    @DrawableRes
    private int getEventIconResId(GitLabReview review) {
        switch (review.state()) {
    ...
    private void formatTitle(final GitLabReview review) {
        int textResId;
        switch (review.state()) {
            case Approved:
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_show_details: {
                GitLabReview review = (GitLabReview) v.getTag();
    ...
    public boolean onMenuItemClick(MenuItem item) {
        GitLabReview review = (GitLabReview) ivMenu.getTag();

        switch (item.getItemId()) {
            case R.id.share:
                IntentUtils.share(mContext, "Pull Request #" + mIssueNumber + " - GitLabReview",
