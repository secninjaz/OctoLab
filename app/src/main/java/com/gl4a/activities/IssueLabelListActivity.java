...
import com.gl4a.adapter.IssueLabelAdapter;
...
import com.gl4a.widget.DividerItemDecoration;
...
public class IssueLabelListActivity extends BaseActivity implements
    ...
    // Fix: cache the resolved projectId so that mutation methods (addLabel/editLabel/deleteLabel)
    // reuse the same ID that was used to load labels, avoiding inconsistent state on retry.
    private long mResolvedProjectId = -1L;
    ...
    private void updateFabVisibility() {
        boolean visible = Gl4Application.get().isAuthorized() && mActionMode == null;
    ...
    private io.reactivex.Single<Long> getProjectIdSingle() {
        // Fix: reuse the resolved projectId from loadLabels() rather than issuing a fresh
        // lookup for every mutation, avoiding potential inconsistency if the lookup result
        // differs across retries.
        if (mResolvedProjectId > 0) {
            return io.reactivex.Single.just(mResolvedProjectId);
        }
        return SingleFactory.getProjectId(mRepoOwner, mRepoName)
                .doOnSuccess(id -> mResolvedProjectId = id);
    }
    ...
    private void deleteLabel(IssueLabelAdapter.EditableLabel label) {
        ...
        getProjectIdSingle()
                .flatMap(projectId -> service.deleteLabel(projectId, label.base().id())
    ...
    private void editLabel(IssueLabelAdapter.EditableLabel label) {
        GitLabLabel oldLabel = label.base();
        String errorMessage = getString(R.string.issue_error_edit_label, oldLabel.name());
        GitLabIssueService service = ServiceFactory.get(GitLabIssueService.class, false);
        Map<String, Object> body = new HashMap<>();
        body.put("new_name", label.editedName != null ? label.editedName : oldLabel.name());
        body.put("color", label.editedColor != null ? label.editedColor : oldLabel.color());

        getProjectIdSingle()
                .flatMap(projectId -> service.editLabel(projectId, oldLabel.id(), body)
                        .map(ApiHelpers::throwOnFailure))
    ...
    private void addLabel(IssueLabelAdapter.EditableLabel label) {
        ...
        body.put("color", label.color());

        getProjectIdSingle()
                .flatMap(projectId -> service.createLabel(projectId, body)
                        .map(ApiHelpers::throwOnFailure))
                .compose(RxUtils.wrapForBackgroundTask(this, R.string.saving_msg, errorMessage))
                .subscribe(result -> {
                    loadLabels(true);
    ...
    private void loadLabels(boolean force) {
        GitLabIssueService service = ServiceFactory.get(GitLabIssueService.class, false);
        // Fix: cache the resolved projectId for reuse in mutation methods.
        SingleFactory.getProjectId(mRepoOwner, mRepoName)
                .doOnSuccess(id -> mResolvedProjectId = id)
                .flatMap(projectId -> service.getLabels(projectId, 1, 100)
                        .map(ApiHelpers::throwOnFailure))
                .map(labels -> {
                    java.util.List<IssueLabelAdapter.EditableLabel> result = new java.util.ArrayList<>();
                    for (GitLabLabel l : labels) result.add(new IssueLabelAdapter.EditableLabel(l));
                    return result;
                })
