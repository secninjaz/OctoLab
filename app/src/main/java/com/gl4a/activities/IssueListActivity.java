...
import com.gl4a.widget.SearchView;
...
import com.gl4a.fragment.LoadingListFragmentBase;
import com.gl4a.fragment.SingleChoiceDialogFragment;
...
public class IssueListActivity extends BaseFragmentPagerActivity implements
    ...
    public static Intent makeIntent(Context context, String repoOwner, String repoName) {
        return makeIntent(context, repoOwner, repoName, -1L, false);
    }

    public static Intent makeIntent(Context context, String repoOwner, String repoName,
            long projectId) {
        return makeIntent(context, repoOwner, repoName, projectId, false);
    }

    public static Intent makeIntent(Context context, String repoOwner, String repoName,
            boolean isPullRequest) {
        return makeIntent(context, repoOwner, repoName, -1L, isPullRequest);
    }

    public static Intent makeIntent(Context context, String repoOwner, String repoName,
            long projectId, boolean isPullRequest) {
    ...
    private List<GitLabLabel> mLabels;
    private List<GitLabMilestone> mMilestones;
    private List<GitLabUser> mAssignees;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!mIsPullRequest && Gl4Application.get().isAuthorized()) {
    ...
    protected String getActionBarSubtitle() {
        return mDisplaySubtitle != null ? mDisplaySubtitle : mRepoOwner + "/" + mRepoName;
    }
    ...
    protected void onInitExtras(Bundle extras) {
        ...
        // Fetch project display name for consistent casing in the subtitle.
        if (mProjectId > 0) fetchDisplaySubtitle(mProjectId);
    }

    private void fetchDisplaySubtitle(long projectId) {
        ServiceFactory.get(com.gl4a.gitlab.service.GitLabProjectService.class, false)
            .getProject(projectId)
            .subscribeOn(io.reactivex.schedulers.Schedulers.io())
            .observeOn(io.reactivex.android.schedulers.AndroidSchedulers.mainThread())
            .subscribe(resp -> {
                if (resp.isSuccessful() && resp.body() != null) {
                    mDisplaySubtitle = resp.body().displayName();
                    androidx.appcompat.app.ActionBar ab = getSupportActionBar();
                    if (ab != null) ab.setSubtitle(mDisplaySubtitle);
                }
            }, err -> { /* best-effort */ });
    }
        ...
        switch (tag) {
            ...
            case "assigneeselect":
                mSelectedAssignee = position == 0 ? null
                        : position == 1 ? "" : mAssignees.get(position - 2).username;
                break;
    ...
    protected Fragment makeFragment(int position) {
        ...
        // Project-level context: use projectId-based API (not personal feed scope).
        // mScope = null signals IssueListFragment to use listIssues(projectId, state, ...)
        // IssueListActivity always has repoOwner/repoName set for project context.
        return IssueListFragment.newInstanceForProject(
                mProjectId, mRepoOwner, mRepoName,
                mSortHelper.getSortMode(), mSortHelper.getSortOrder(),
                getIssueState(position), emptyTextResId, mIsPullRequest,
                mSelectedLabel, mSelectedMilestone, mSelectedAssignee, mSearchQuery);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_ISSUE_CREATE) {
            if (resultCode == Activity.RESULT_OK && data.hasExtra("issue_iid")) {
                int issueIid = data.getIntExtra("issue_iid", 0);
                // Use project_id from the creation result if our local mProjectId is unknown (-1).
                // IssueEditActivity now returns project_id alongside issue_iid.
                long projectId = data.getLongExtra("project_id", mProjectId);
                if (projectId <= 0) projectId = mProjectId;
                Intent intent = IssueActivity.makeIntent(this, mRepoOwner, mRepoName,
                        projectId, issueIid, null);
    ...
    public void onClick(View view) {
        Intent intent = IssueEditActivity.makeCreateIntent(this, mRepoOwner, mRepoName, mProjectId);
    ...
    private String getIssueState(int position) {
        switch (position) {
            ...
            case 2:
                // "merged" is only valid for MRs; for issues use "all"
                return mIsPullRequest ? ApiHelpers.IssueState.MERGED : "all";
            default:
    ...
    private void showMilestonesDialog() {
        ...
        for (GitLabMilestone milestone : mMilestones) {
    ...
    private void showAssigneesDialog() {
        ...
        for (GitLabUser user : mAssignees) {
    ...
    private void filterAssignee() {
        if (mAssignees != null) {
            ...
            var service = ServiceFactory.getForFullPagedLists(GitLabIssueService.class, false);
            // GitLab: project members are potential assignees; fall back to getLabels/project members
            // TODO: GitLab does not have a direct assignee listing endpoint — use project members
            var projectService = ServiceFactory.getForFullPagedLists(
                    com.gl4a.gitlab.service.GitLabProjectService.class, false);
            registerTemporarySubscription(projectService.getAllMembers(mProjectId, 1, 100)
    ...
    private void filterMilestone() {
        if (mMilestones != null) {
            ...
            var service = ServiceFactory.getForFullPagedLists(GitLabIssueService.class, false);
            registerTemporarySubscription(service.getMilestones(mProjectId, "active", 1, 100)
    ...
    private void filterLabel() {
        if (mLabels != null) {
            ...
            var service = ServiceFactory.getForFullPagedLists(GitLabIssueService.class, false);
            registerTemporarySubscription(service.getLabels(mProjectId, 1, 100)
