...
public class RxUtils {
                ...
                if (error instanceof IOException) {
                    ...
                    snackbar
                        ...
                        .setAction(
                            ...
                            view -> IntentUtils.copyToClipboard(context, "GitLab error", error.getMessage())
                        );
                } else {
                    snackbar
                        .setText(context.getString(R.string.snackbar_application_error, errorMessage))
    ...
    public static <T> Single<Response<GitLabPage<T>>> searchPageAdapter(Single<Response<Object>> upstream) {
        // Stub: GitLab search responses use GitLabPage directly; this adapter is a no-op.
        return upstream.map(response -> {
            if (response.isSuccessful() && response.body() instanceof GitLabPage) {
                return Response.success((GitLabPage<T>) response.body());
            }
            return Response.success(new GitLabPage<>(java.util.Collections.emptyList(), 1, 0, 1, 0));
        });
    }

    @SuppressWarnings("unchecked")
    public static <U, D> Single<Response<GitLabPage<D>>> searchPageAdapter(Single<Response<Object>> upstream, Function<U, D> mapper) {
        return searchPageAdapter(upstream);
    }
