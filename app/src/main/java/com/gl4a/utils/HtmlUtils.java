...
public class HtmlUtils {
    ...
    /**
     * Convert GitLab Flavored Markdown to HTML suitable for Android's Html.fromHtml().
     * Supports headings, bold, italic, strikethrough, inline code, fenced code blocks,
     * unordered/ordered lists, task lists, blockquotes, images, links, and horizontal rules.
     * Code blocks are extracted first so markdown syntax inside them is not processed.
     */
    public static String markdownToHtml(String markdown) {
        if (markdown == null) return "";

        java.util.ArrayList<String> codeBlocks = new java.util.ArrayList<>();
        java.util.ArrayList<String> inlineCodes = new java.util.ArrayList<>();

        // Step 1: extract fenced code blocks (protect their content)
        Matcher m = Pattern
                .compile("(?s)```[^\n]*\n(.*?)```").matcher(markdown);
        ...
        // Step 3: HTML-escape everything else
        s = s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;");

        // Step 4: apply markdown transforms (order matters)

        // Headings
        s = s.replaceAll("(?m)^######[ \t]+(.*?)$", "<h6>$1</h6>");
        s = s.replaceAll("(?m)^#####[ \t]+(.*?)$",  "<h5>$1</h5>");
        s = s.replaceAll("(?m)^####[ \t]+(.*?)$",   "<h4>$1</h4>");
        s = s.replaceAll("(?m)^###[ \t]+(.*?)$",    "<h3>$1</h3>");
        s = s.replaceAll("(?m)^##[ \t]+(.*?)$",     "<h2>$1</h2>");
        s = s.replaceAll("(?m)^#[ \t]+(.*?)$",      "<h1>$1</h1>");

        // Blockquotes (&gt; because we already HTML-escaped)
        s = s.replaceAll("(?m)^&gt;[ \t]?(.*?)$", "<blockquote>$1</blockquote>");

        // Horizontal rules
        s = s.replaceAll("(?m)^(-{3,}|\\*{3,}|_{3,})[ \t]*$", "<hr/>");

        // Task list items (before unordered list)
        s = s.replaceAll("(?m)^[ \t]*[-*][ \t]+\\[ \\][ \t]+(.*?)$",
                "<li>☐ $1</li>");
        s = s.replaceAll("(?m)^[ \t]*[-*][ \t]+\\[x\\][ \t]+(.*?)$",
                "<li>☑ $1</li>");
        ...
        s = s.replaceAll("(?m)^[ \t]*\\d+\\.[ \t]+(.*?)$", "<li>$1</li>");

        // Unordered list items
        s = s.replaceAll("(?m)^[ \t]*[-*][ \t]+(.*?)$", "<li>$1</li>");

        // Wrap consecutive <li> lines in <ul>
        s = s.replaceAll("(?m)((?:^<li>.*?</li>$\\n?)+)", "<ul>$1</ul>\n");
        ...
        // Italic: *text* or _text_ (not preceded by same char)
        s = s.replaceAll("(?<![*])\\*(?![*])(.+?)(?<![*])\\*(?![*])", "<i>$1</i>");
        s = s.replaceAll("(?<!_)_(?!_)(.+?)(?<!_)_(?!_)", "<i>$1</i>");
        ...
        // Paragraphs: blank lines → paragraph break, single newlines → br
        s = s.replaceAll("(\n[ \t]*){2,}", "<p>");
        s = s.replace("\n", "<br/>");

        // Step 5: restore code blocks and inline codes
        for (int i = 0; i < codeBlocks.size(); i++) {
            s = s.replace("CB" + i + "", codeBlocks.get(i));
        }
        for (int i = 0; i < inlineCodes.size(); i++) {
            s = s.replace("IC" + i + "", inlineCodes.get(i));
        }
    ...
    public static String rewriteRelativeUrls(final String html, final String repoUser,
        ...
        final String baseUrl = instanceUrl + "/" + repoUser + "/" + repoName + "/-/blob/" + ref + "/" + folderPath;
        String rewrittenHtml = rewriteUrlsInAttribute("href", html, baseUrl);

        final String baseUrlForImages = instanceUrl + "/" + repoUser + "/" + repoName + "/-/raw/" + ref + "/" + folderPath;
    ...
    private static class HtmlToSpannedConverter implements ContentHandler {
        ...
        private void handleStartTag(String tag, Attributes attributes) {
            ...
            if (tag.equalsIgnoreCase("br")) {
                ...
                appendNewlines(2);
                start(new Table());
            } else if (tag.equalsIgnoreCase("thead")) {
                start(new TableHead());
            } else if (tag.equalsIgnoreCase("tbody")) {
                // separator line after header already appended on </thead>
        ...
        private void handleEndTag(String tag) {
            if (tag.equalsIgnoreCase("br")) {
                ...
                // Append separator row after header
                mSpannableStringBuilder.append('\n');
                mSpannableStringBuilder.append("\u251c\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2524");
            } else if (tag.equalsIgnoreCase("tbody")) {
                // nothing
