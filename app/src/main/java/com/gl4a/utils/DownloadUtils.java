...
public class DownloadUtils {
    public static void enqueueDownloadWithPermissionCheck(final BaseActivity activity,
            final GitLabRelease.Asset asset) {
        // Asset URLs (GitLab Generic Packages registry) support PRIVATE-TOKEN directly.
        // The packages API URL returns the file with Content-Length in one request — no redirect.
        // DownloadManager with PRIVATE-TOKEN header handles this correctly.
        String url = asset.browserDownloadUrl();
        if (url == null) return;
        handleDownloadPermissionCheck(activity, () ->
            enqueueDownload(activity, android.net.Uri.parse(url),
                    asset.name(), null, "application/vnd.android.package-archive",
                    null, false, true /* addAuthHeader */));
    }
    ...
    private static void enqueueDownload(Context context, Uri uri, String fileName,
        ...
        final String token = Gl4Application.get().getAuthToken();
        if (addAuthHeader && token != null) {
            // Use PRIVATE-TOKEN for GitLab (PAT) or Authorization: Bearer for OAuth
            String tokenType = Gl4Application.get().getAuthTokenType();
            if (Gl4Application.TOKEN_TYPE_OAUTH.equals(tokenType)) {
                request.addRequestHeader("Authorization", "Bearer " + token);
            } else {
                request.addRequestHeader("PRIVATE-TOKEN", token);
            }
        }
        if (mimeType != null) {
    ...
    private static void enqueueDownload(final Context context, final GitLabRelease.Asset asset) {
        ...
        //  the Android Object Manager) and you disable automatic following of redirects when you
        ...
        //  Second, you use the Android Object Manager to download the asset from the URL that's
        ...
        if (token != null) {
            ...
            if (Gl4Application.TOKEN_TYPE_OAUTH.equals(tokenType)) {
                ...
                requestBuilder.addHeader("PRIVATE-TOKEN", token);
            }
        }
        ...
        client.newCall(requestBuilder.build()).enqueue(new Callback() {
            private void completeDownload(final String url) {
                handler.post(() -> {
                    enqueueDownload(context, url, asset.name(), null, null,
                            "application/octet-stream", false);
                });
            }
            @Override
            public void onFailure(Call call, IOException e) {
                completeDownload(asset.browserDownloadUrl());
            }
